...
def finns(p,value): #wtf not working
    ...
    if value == p.value:
        return True
    if value < p.value: #om värdet är mindre än föräldern
        return finns(p.left,value)  #kallar på sig själv men nu med noden som var till vänster om föräldern, som förälder
    if value > p.value: #samma sak som ovan men kollar högra delen om värder är högre än förälderns värde
        return finns(p.right, value)

def skriv(p):
     # Funktion som gör själva jobbet att skriva ut trädet (inorder)
     if p != None: 
         skriv(p.left)
         print(p.value)
         skriv(p.right)
